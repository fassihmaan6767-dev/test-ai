...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser && currentUser.email === 'talkwithfasih@gmail.com') {
        setUser(currentUser);
        const isVerified = sessionStorage.getItem('admin_otp_verified');
        if (isVerified === 'true') {
          setOtpVerified(true);
          setShowOtpScreen(false);
        } else {
    ...
    if (authMode === 'forgot') {
      return (
        <form onSubmit={async (e) => {
          ...
          <div>
            <label className="block text-xs font-medium text-[#4A3E33] mb-1">Reset Password for Admin</label>
            <input
              type="email"
              required
              value={email}
          ...
          <button
            ...
            className="w-full py-3 bg-[#241E1A] hover:bg-[#3D332D] text-white rounded-xl text-sm font-medium transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
          >
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#f4eee6] to-[#e6dcd0] flex flex-col justify-center items-center p-6 relative overflow-hidden font-sans">
        ...
        >
          <div className="flex items-center justify-center w-14 h-14 bg-gradient-to-b from-white to-[#f0e8dc] border border-white shadow-sm rounded-full mx-auto mb-6 text-[#4A3E33]">
            {showOtpScreen ? <ShieldCheck className="w-6 h-6 text-emerald-600" /> : <ShieldCheck className="w-6 h-6" />}
          </div>

          <div className="text-center mb-8">
            <h1 className="font-serif text-3xl font-medium text-[#1A1A1A] tracking-tight">
              {showOtpScreen ? '2-Step Verification' : 'Admin Portal'}
            </h1>
            <p className="text-xs text-[#8A7B6E] mt-2 tracking-wide uppercase">
              {showOtpScreen ? 'Enter 6-Digit OTP' : 'Secure Access Only'}
            </p>
          </div>
          ...
          {showOtpScreen && user ? (
            <form onSubmit={handleVerifyOtp} className="space-y-4 text-center">
              <div>
                <label className="block text-xs font-medium text-[#4A3E33] mb-2">We sent a verification code to <strong>{user.email}</strong></label>
                <input
                  type="text"
                  required
                  maxLength={6}
                  ...
                  className="w-full text-center tracking-[12px] font-mono text-2xl px-4 py-4 bg-white border border-[#d6c7b4] rounded-xl outline-none focus:border-[#4A3E33] text-[#1A1A1A]"
                />
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-[#dfd2c0] flex justify-between items-center px-2">
                <button
                  type="button"
                  onClick={handleSendOtp}
                  disabled={otpSending}
                  className="text-xs text-[#6B5E52] hover:text-[#1A1A1A] underline cursor-pointer disabled:opacity-50"
                >
